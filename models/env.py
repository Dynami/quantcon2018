from functs.functions import cycle, scale, lowPassFilter, market_meanness_index
import pandas as pd
import numpy as np
import talib
import traceback
...
class Game(object):
    ...
    def __init__(self, df, look_back=20, max_game_len=1000,
                 ...
                 n_last_bars_in_state=5, debug=False):
        self.debug = debug
        self.df = df
    ...
    def _update_state(self):
        #if self.debug: print("Game::_update_state()")

        self.curr_idx += 1
        if self.debug: print('Game::_update_state()', self.curr_idx)
        self.start_time = self.df.index[self.start_idx]
        self.curr_time = self.df.index[self.curr_idx]
    ...
    def old__update_state(self, action):
    ...
    def _assemble_state(self):
        ...
        for c in candles:
            try:
                ...
                # f1 = lowPassFilter(candles[c]['close'], 5)[-self.n_last_bars:]
                # f2 = lowPassFilter(candles[c]['close'], 15)[-self.n_last_bars:]
                # self.state = np.append(self.state, scale((f1 - f2) / f2))

                #tmp = candles[c]['close'][-self.n_last_bars:]-f1
                #tmp = np.append(state, scale(tmp))
                ...
                tmp = talib.PLUS_DM(candles[c]['high'], candles[c]['low'])[-self.n_last_bars:]
                self.state = np.append(self.state, scale(tmp))

                tmp = talib.MINUS_DM(candles[c]['high'], candles[c]['low'])[-self.n_last_bars:]
                self.state = np.append(self.state, scale(tmp))

                tmp = talib.ADX(candles[c]['high'], candles[c]['low'], candles[c]['close'])[-self.n_last_bars:]
                self.state = np.append(self.state, scale(tmp)) #, min=0., max=100.

                tmp = talib.ADXR(candles[c]['high'], candles[c]['low'], candles[c]['close'])[-self.n_last_bars:]
                self.state = np.append(self.state, scale(tmp)) #, min=0., max=100.
                ##                               candles[c]['low'],
    ...
    def observe(self, next_state=True):
        if self.debug:
            print("Game::observe()")
        if next_state:
            self._get_last_N_timebars()
    ...
    def _act(self, action, force_exit=False):
        """
